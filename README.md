...
> 🚀 拒绝多应用切换，享受原生级问答体验！一键并发提问，自定义网页视口，轻松实现多个大模型回答的同屏对比与智能聚合总结。
...
市面上的很多聚合类 AI 软件仅仅是单纯的网页加载套壳，体验割裂。**AMAi** 旨在为你提供**媲美原生的问答体验**。你不需要下载十几个不同的 AI App，也不需要为了对比各家模型的回答而来回复制粘贴同一个问题。在一个统一、现代化的界面中，AMAi 帮你将问题一键分发，轻松实现多个 AI 大模型回答的同屏对比，并支持一键聚合总结各方回答。

<p align="center">
  <img src="assets/视口自定义裁剪.gif" alt="视口自定义裁剪" width="320"  style="margin-right:50px;" />
  <img src="assets/一键并发提问.gif" alt="一键并发提问" width="320" />
</p>
...
| 🧠 **应答聚合总结** | **核心新特性**：一键汇总多个大模型的回答，通过自定义聚合 Prompt 模版，智能合并为统一的综合回复，省去逐个浏览的繁琐。 |
...
| 🛠️ **极强自定义扩展** | 内置管理面板，支持自定义添加各种 AI 大模型。采用通用的策略模式（TextFiller + Pipeline），通过 XPath 路径配置即可完美兼容标准 DOM 及 React/Slate.js 结构的网站，支持为每个标签页单独配置填入策略。 |
...
- [x] **AI 回答聚合总结**：一键汇总多个大模型的回答，通过自定义聚合 Prompt 模版智能合并为综合回复（v1.2.0 已实现）。
- [x] **注入策略架构重构**：合并为 TextFiller + Pipeline 模式，支持为每个标签页单独配置填入策略（v1.2.0 已实现）。
